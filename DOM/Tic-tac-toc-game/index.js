...
function initializeGame() {
  ...
  document.querySelector("h2").innerHTML =
    'Turn of: <span id="turnPlayer"></span>';
  updateTitle();
...
// to remove the event after using the region
function disableRegion(element) {
  element.classList.remove("cursor-pointer");
  element.removeEventListener("click", handleBoardClick);
}

// function that avaluetes if a winning region
...
// functoi that show a win message and change the regions
...
// function that handlw the click of each region
function handleBoardClick(ev) {
  const span = ev.currentTarget;
  const region = span.dataset.region; // comes as N.N.N.N
  ...
  if (winRegions.length > 0) {
    ...
    document.querySelector("h2").innerHTML = "No one won!";
  }
}
