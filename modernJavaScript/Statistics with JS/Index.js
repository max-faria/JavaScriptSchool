...
const mediana = (...numbers) => {
  ...
  const midle = Math.floor(ordenedNumbers.length / 2);
  if (ordenedNumbers.length % 2 === 0) {
    return (ordenedNumbers[midle - 1] + ordenedNumbers[midle]) / 2;
  } else {
    return ordenedNumbers[midle];
  }
};

console.log(`Median with arrow function is: ${mediana(2, 5, 99, 4, 42, 7)}`);
console.log(`Median with arrow function is: ${mediana(15, 14, 8, 7, 3)}`);
...
console.log(`Mode is: ${mode(1, 1, 2, 4, 5, 7, 8, 9, 9, 9, 5, 4, 9, 9, 0)}`);
